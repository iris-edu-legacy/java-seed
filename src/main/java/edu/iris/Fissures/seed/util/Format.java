...
//lifted from JWeed v2.9.8 with the following changes:
// Changed to print space instead of '.' when precision is 0 in 'fixed_format()'.
// Added 'format(Object obj)' method.
// Increased precision for 'G'/'g' format when no precision is specified.
...
{ /**
   ...
   */
    ...
    * @param obj the Object for format
   ...
   public String format(Object obj)
   {
       ...
       case 'c':
           ...
           return format(((Character) obj).charValue());
       case 'e':
       case 'f':
       case 'g':
       case 'E':
       ...
       if (obj instanceof Integer || obj instanceof Short)
           return format(((Number) obj).intValue());
       return format(((Number) obj).longValue());
   }
   ...
   {  boolean removeTrailing
      ...
      if (fPrecision < 0) {
          ...
          fPrecision -= Long.toString(whole).length() - 1;
          if (fPrecision < precision)
              fPrecision = precision;
      }
      double factor = 1;
      String leading_zeroes = "";
      for (int i = 1; i <= fPrecision && factor <= 0x7FFFFFFFFFFFFFFFL; i++)
      {  factor *= 10;
      ...
      z = "." + z.substring(z.length() - fPrecision, z.length());
...
// the following two commented lines where changed by tjm on 2-8-1999
// in order to get 2 digit exponents
//       String p = "000";
      String p = "00";
      if (e >= 0)
      {  f = f + "+";
...
//       return f + p.substring(p.length() - 2, p.length());
      return f + p.substring(p.length() - 2, p.length());
   }
